...
public class CartServiceClient
{
    ...
    public async Task<CartModel> GetUserCartAsync(long UserId)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            {
                return JsonSerializer.Deserialize<CartModel>(responseContent, 
    ...
    public async Task<int> UpdateQuantityAsync(long CartId, int ItemId, int Quantity)
    {
        var status = await _client.GetFromJsonAsync<int>("cart/UpdateQuantity/" + CartId + "/" + ItemId + "/" + Quantity);
        return status;
    }

    public async Task<int> GetCartItemCounAsync(long UserId)
    {
        try
        {
